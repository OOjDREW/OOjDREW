 ...
 *
 * The call format is replace(?result, input1,input2,input3).
 *
 * <p>Title: OO jDREW</p>
 *
 ...
 *
 */
public class ReplaceBuiltin implements Builtin {
    private int symbol = SymbolTable.internSymbol("replace");
    ...
    public DefiniteClause buildResult(Term t) {
        ...
        if (p2.getType() != Types.ISTRING || p3.getType() != Types.ISTRING || p4.getType() != Types.ISTRING) {
            return null;
        }
        ...
        v.add(p3);
        v.add(p4);
        String results = p2s.replaceAll(p3s, p4s);
        ...
        Term roid = new Term(SymbolTable.internSymbol("$jdrew-replace-" + p3s),
