...
public class SubstringBeforeBuiltin implements Builtin {
    ...
    public DefiniteClause buildResult(Term t) {
        ...
        v.add(p2);
        v.add(p3);
        String results = p2s.contains(p3s) ? p2s.substring(0, p2s.indexOf(p3s)):p2s.substring(p2s.length()) ;
        ...
        Term roid = new Term(SymbolTable.internSymbol("$jdrew-substringbefore-" + p3s),
