 ...
 *
 * The call format is add(?result, base,exponent).
 *
 * <p>Title: OO jDREW</p>
 *
 ...
 * @version 0.89
 */
public class PowBuiltin implements Builtin {
    private int symbol = SymbolTable.internSymbol("pow");
    ...
    public DefiniteClause buildResult(Term t) {
        ...
        if (p2.getType() == Types.IINTEGER && p3.getType() == Types.IINTEGER) {
            ...
            String results = "" + (long) (Math.pow(p2i, p3i));
            ...
            String results = "" + (Math.pow(p2d, p3d));
            r1 = new Term(SymbolTable.internSymbol(results),
        ...
        Term roid = new Term(SymbolTable.internSymbol("$jdrew-pow-"
                                                      + p2s + "^" + p3s),
