 ...
 *
 * Calling format containsIgnoreCase(?input1, input2)
 * 
 * Satisfied iff the first argument contains the second argument (not case sensitive).
 * <p>Title: OO jDREW</p>
 *
 ...
 * @version 0.89
 */
public class ContainsIgnoreCaseBuiltin implements Builtin {
    private int symbol = SymbolTable.internSymbol("containsIgnoreCase");
    ...
    public DefiniteClause buildResult(Term t) {
        ...
        String p1s = p1.getSymbolString().toLowerCase();
        String p2s = p2.getSymbolString().toLowerCase();

        if (p1s.indexOf(p2s) == -1) {
            return null;
        }

        Term roid = new Term(SymbolTable.internSymbol("$jdrew-containsIC-"
                                                      + p1s + "-" + p2s),
