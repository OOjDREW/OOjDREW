 ...
 *
 * The call format is floor(?result, input:Real).
 *
 * <p>Title: OO jDREW</p>
 *
 ...
 * @version 0.89
 */
public class FloorBuiltin implements Builtin {
    private int symbol = SymbolTable.internSymbol("floor");
    ...
    public DefiniteClause buildResult(Term t) {
        ...
        if (p2.getType() == Types.IINTEGER || p2.getType() == Types.IFLOAT) {
            ...
            String results = "" + (long) Math.floor(d);
        ...
        Term roid = new Term(SymbolTable.internSymbol("$jdrew-floor-" + p2s),
